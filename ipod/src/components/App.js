...
// song_cover images
import song1Img from "../static/song1img.webp";
import song2Img from "../static/song2img.webp";
...
class App extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      songItems: ["Sapne Bade - BantiGG ", "Tera Ruthana"], 
  ...
  seekSongForward = (e) => {
    ...
    if (e.detail.interval < 250) {
      ...
      this.setState({
        ...
        // Ensure it plays only after previous pause is resolved
        setTimeout(() => {
    ...
    // Case 2: Longer interval → seek forward in current song
    } else if (e.detail.interval >= 250 && e.detail.interval < 10000) {
      const interval = e.detail.interval / 100;
      const newTime = this.state.audio.currentTime + interval;
      if (newTime < this.state.audio.duration) {
        this.state.audio.currentTime = newTime;
      } else {
        this.state.audio.currentTime = this.state.audio.duration - 1;
      }
    }
  }
